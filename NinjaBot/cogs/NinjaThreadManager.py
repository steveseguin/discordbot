...
class NinjaThreadManager(commands.Cog):
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        if isinstance(message.channel, discord.Thread) and message.reference and message.reference.message_id and self.ai:
            try:
    ...
    async def title(self, interaction: discord.Interaction, new_title: str | None = None) -> None:
        if new_title:
            await self._title(interaction, new_title)
        else:
            await interaction.response.send_modal(ThreadTitleChangeModal(self))
    ...
    def _getThreadTitle(self, msg) -> str:
        message = msg.content
