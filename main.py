...
class NinjaBot(commands.Bot):
    ...
    async def on_ready(self) -> None:
        ...
        # Copy NinjaAI.py to the parent directory to make it importable
        try:
            import shutil
            cogs_dir = LOCALDIR / "cogs"
            ai_file = cogs_dir / "NinjaAI.py"
            if ai_file.exists():
                shutil.copy(ai_file, LOCALDIR / "NinjaAI.py")
                logger.info("Copied NinjaAI.py to parent directory for import")
        except Exception as e:
            logger.error(f"Failed to copy NinjaAI.py: {e}")
